...
function LineChartWrapper({titleData, currency, coinID}) {
    ...
    if (isError && !isFetching) {
        return <div className="page-container"><ReloadSignal refreshActionName={"loading chart"} refreshClickFn={refetch} /></div>
    }
    ...
    <>      
        <p className="chart-title">
            <span className="title">{titleData[chartInfoType.value].name}:</span> {titleData[chartInfoType.value].data}
        </p>
        ...
        <div className="chart-container">
            {
                ...
                <LineChart coinID="Bitcoin" infoType={chartInfoType} chartData={coinChartData} darkTheme={darkTheme} extraOptions={extraChartOptions} />
                <div className="btns-container">
                    {
                        chartDataTypes.map((type) => {
                            return <ChartTypeButton key={type.value} buttonValue={type} onClickCallback={handleChartButtonClick} referencedStateValue={chartInfoType} />
                        })
                    }
                </div>
                </>
...
function ChartTypeButton({buttonValue, onClickCallback, referencedStateValue,}) {
    return (
        <button className={`btn chart-type-btn ${referencedStateValue.value == buttonValue.value && 'active-chart-type-btn'}`} value={JSON.stringify(buttonValue)} onClick={onClickCallback} type="button" aria-label={`view ${buttonValue.name} chart`}>
            {buttonValue.name}
        </button>
    )
}