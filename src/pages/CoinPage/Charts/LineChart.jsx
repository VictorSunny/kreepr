...
const ResetZoomIcon = lazy(() => import('../../../assets/icons/four-arrows-inside-line-icon.svg?react'))
const PanLeftIcon = lazy(() => import('../../../assets/icons/chevron-direction-left-round-outline-icon.svg?react'))
const PanRightIcon = lazy(() => import('../../../assets/icons/chevron-direction-right-round-outline-icon.svg?react'))
...
function LineChart({infoType, chartData, darkTheme, extraOptions}) {
    ...
    const data = {
        datasets: [{
            label: 'price',
            data: priceData,
            backgroundColor: darkTheme ? extraOptions.colors.dark.background : extraOptions.colors.light.background,
            borderWidth: isMobile ? 1 : 1.5,
            borderColor: darkTheme ? extraOptions.colors.dark.chartLine : extraOptions.colors.light.chartLine,
            tension: 0.4,
            font: {
                family: 'monospace',
                size: 20
            ...
            y: {
                ...
                ticks: {
                    ...
                    callback: (value, index) => {
                        if (index === 0) {return ''} return currencyShortener.format(value);
                    }
                },
            }
        },
        plugins: {
            ...
            tooltip: {
                ...
                callbacks: {
                    label: (context) => {
                        return (
                            `${String(infoType.name)}: ${tooltipCurrencyFormatter.format(context.raw[1])}`
                        )
                    },
                }
            },
            ...
            <div className="chart-zoom-btns-container">
                {
                    (isDesktop || isTablet) &&
                    <>
                        <button className="chart-zoom-btn btn" onClick={panLeft} type="button" aria-label="pan chart left">
                    <PanLeftIcon className="chart-zoom-icon" loading="auto" alt="arrow left icon"/>
                    </button>
                    <button className="chart-zoom-btn btn" onClick={panRight} type="button" aria-label="pan chart right">
                        <PanRightIcon className="chart-zoom-icon" loading="auto" alt="arrow right icon"/>
                    </button>
                    <button className="chart-zoom-btn btn" onClick={zoomIn} type="button" aria-label="zoom into chart">
                        <ZoomInIcon className="chart-zoom-icon" loading="auto" alt="zoom in icon"/>
                    </button>
                    <button className="chart-zoom-btn btn" onClick={zoomOut} type="button" aria-label="zoom out chart">
                        <ZoomOutIcon className="chart-zoom-icon" loading="auto" alt="zoom out icon"/>
                    </button>
                    </>
                }
                <button className="chart-zoom-btn btn" onClick={resetZoom} type="button" aria-label="reset chart zoom">
