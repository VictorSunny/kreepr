...
function CoinPage() {
    ...
    if (isFetching) {
        return <div className="page-container"><LineLoadingSignal /></div>
    }
    // return value if initial api fetch fails
    if (isError && !isFetching) {
        return <div className="page-container"><ReloadSignal refreshActionName={"coin information"} refreshClickFn={refetch} /></div>
    }
    ...
    const coinPrice = currencyFormatter.format(coinData?.market_data.current_price[preferredCurrency.value])
    const coinMarketCap = currencyFormatter.format(coinData?.market_data.market_cap[preferredCurrency.value])
    const coinVolume = currencyFormatter.format(coinData?.market_data.total_volume[preferredCurrency.value])
        ...
        <div className="page-container coin-page-container">
            <div className="sticky-intro coin-sticky-intro">
                <div className="coin-intro-details-container">
                    <div className="intro-logo-name">
                        <img className="coin-logo" src={coinData?.image.large} loading="eager" alt="coin logo"></img>
            ...
            <div id="coin-page-main-content">
                <div className="section line-chart-section section-1">
                    ...
                    <LineChartWrapper className="line-chart" titleData={chartTitleData} currency={preferredCurrency.value} coinID={coinID} />
                </div>

                <div className="section currencies-section section-2">
                    <div className="sub-section">
                        <p className="coin-rank">
                            Rank: #{coinMarketCapRank}
                        </p>
                        <p>
                ...
                <div className="section about-section section-4">
                    ...
                    <div className="coin-votes">
                        <div>
                            ...
                            <span>
                                {
                                coinUpvotesPercent || coinUpvotesPercent == 0 &&
                                `${coinUpvotesPercent}%` ||
                                '?'
                                }
                                </span>
                        ...
                        <div>
                            <DislikeIcon className="vote-icon negative-vote-icon" loading="lazy" alt="dislikes icon" />
                            <span>
                                {
                                coinDownvotesPercent || coinDownvotesPercent == 0 &&
                ...
                <div className="section links-section section-5">
                    <div className="coin-links-container">
                        <div>
                            ...
                            <div>
                                <a target="_blank" rel="noopener noreferrer" href={coinHomepageLink} className="btn coin-link">
                                    Homepage
                                </a>
                                <a target="_blank" rel="noopener noreferrer" href={coinWhitepaperLink} className="btn coin-link">
                                    Whitepaper
                                </a>
                            </div>
                        </div>
                            ...
                            <ol className="coin-sub-links-list">
                                {
                                    ...
                                    coinBlockchainSites.map((siteLink, index) => {
                                        return (
                                            <li key={index}>
                                                <a target="_blank" rel="noopener noreferrer" className="btn coin-link" href={siteLink}>
                                                    {urlParser(siteLink)}
                                                </a>
                                            </li>
                                        )
