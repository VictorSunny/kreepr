...
export default function SearchBar({mobileTriggered}) {
    ...
    return (
        <div id="search-bar-container" onClick={clickedSearch}>
            <input ref={searchBarInput} onBlur={hideSearchPopup} onFocus={() => {setSearchEngaged(true)}} onChange={searchAction} type='text' placeholder="search for coins..." id="search-bar"></input>
            {(userIsTyping || searchClicked ) && <SearchPopup searchQuery={searchQuery} userIsTyping={userIsTyping} userHasTyped={userHasTyped} />}
...
function SearchPopup({searchQuery, userIsTyping, userHasTyped}) {
    ...
    if (isFetching) {
        return <div className="search-popup"><LineLoadingSignal /></div>
    }
    if (isError && !isFetching) {
        return <div className="search-popup"><ReloadSignal refreshActionName={"loading coins"} refreshClickFn={refetch} /></div>
    }
